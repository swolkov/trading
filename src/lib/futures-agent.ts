...
import { getPlanContextForGrading, escalateToAdvisor, shouldEscalate, getDailyPlan } from "./advisor";
...
export async function runFuturesAgent(opts: { registryOnly?: boolean } = {}): Promise<{
  ...
  for (const symbol of symbolsToTrade) {
    ...
    // AI confirmation — two-tier system:
    // 1. Worker model (Opus) does initial grade
    // 2. Fable 5 advisor escalation for borderline A/B grades on live
    let aiConviction = "";
    // Fetch today's strategic plan context (generated by Fable 5 at premarket)
    let planContext = "";
    try { planContext = await getPlanContextForGrading(symbol); } catch { /* plan optional */ }
    try {
...
Reply ONLY with JSON: {"agree": true/false, "conviction": "A+"|"A"|"B"|"C", "reasoning": "one sentence"}`;
      ...
      const preWorkerConfidence = setup.confidence;
      ...
      // ─── FABLE 5 ESCALATION: Borderline grades get a second opinion ───
      const mode = tradingMode === "paper" ? "demo" : "live";
      const planData = planContext ? await getDailyPlan() : null;
      const planConflict = (planData?.bias === "bearish" && setup.direction === "long") || (planData?.bias === "bullish" && setup.direction === "short");
      if (ai.agree && shouldEscalate({
        workerGrade: ai.conviction || "A",
        ...
        details.push(`  ESCALATING to advisor (borderline ${ai.conviction} on ${mode})...`);
        const escalation = await escalateToAdvisor({
          symbol, direction: setup.direction, setupType: setup.type,
          reasoning: setup.reasoning, price,
          stopDistance: setup.stopDistance, targetDistance: setup.targetDistance,
          rsi: currentRSI || 50, atr: currentATR, vwap: vwapData.vwap,
          trend15, dayType, session,
          workerGrade: ai.conviction || "A", workerReasoning: ai.reasoning || "",
          technicalScore: preWorkerConfidence, mode: mode as "live" | "demo",
        });
        if (escalation) {
          details.push(`  ADVISOR VERDICT: ${escalation.finalGrade} — ${escalation.reasoning}${escalation.overrideWorker ? " [OVERRIDES WORKER]" : ""}`);
          if (escalation.overrideWorker) {
            // Reset to pre-worker confidence, then apply advisor's grade cleanly
            setup.confidence = preWorkerConfidence;
            if (escalation.finalGrade === "A+") {
              aiConviction = "A+";
              setup.confidence += 30;
            } else if (escalation.finalGrade === "A") {
              aiConviction = "A";
              setup.confidence += 20;
            } else if (escalation.finalGrade === "B") {
              aiConviction = "B";
              if (mode === "live") { setup.confidence -= 15; }
              else { setup.confidence += 10; }
            } else {
              // C from advisor = kill
              aiConviction = "";
              setup.confidence -= 25;
            }
            details.push(`  Confidence: ${preWorkerConfidence} → ${setup.confidence} (advisor override, was ${ai.conviction}→${escalation.finalGrade})`);
          }
        }
      }
    } catch {
