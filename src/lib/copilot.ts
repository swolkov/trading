...
import { gradeAsk } from "@/lib/trade-grades";
...
export async function copilotPoll(nowMs = Date.now(), readOrdersThisPoll = true): Promise<{ ok: boolean; posted: number; note: string }> {
  ...
  try {
    ...
    const { state: next, messages, gradeAsks } = step(state, { nowMs, positions: room, orders: tracking ? orders : null, prices, fills, records, rejectedStops: orders ? lastRejected : [] });
