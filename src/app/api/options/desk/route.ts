...
export async function GET(){
  const [rows,account,context]=await Promise.all([
    prisma.agentConfig.findMany({where:{key:{in:[OPTIONS_RESEARCH_KEY,OPTIONS_MAX_LOSS_KEY,"options_live_armed","options_live_integration_verified","options_live_guardian_ok_at",OPTIONS_DIRECT_STATUS_KEY]}}}),readAccountSnapshot(),news()]);
  ...
  const armed=values.options_live_armed==="true", verified=values.options_live_integration_verified==="true";
  const guardianAt=Date.parse(values.options_live_guardian_ok_at??""), guardianFresh=Number.isFinite(guardianAt)&&Date.now()-guardianAt<10*60_000;
  const blockers:string[]=[];
  if(!armed)blockers.push("Live desk is disarmed (arm it on the Live Account page)");
