import { OPTIONS_DIRECT_STATUS_KEY, directConnectionView } from "@/lib/options-direct-collector";
...
export async function GET(){
  const [rows,account,context]=await Promise.all([
    prisma.agentConfig.findMany({where:{key:{in:[OPTIONS_RESEARCH_KEY,OPTIONS_MAX_LOSS_KEY,"options_live_armed",OPTIONS_DIRECT_STATUS_KEY]}}}),readAccountSnapshot(),news()]);
  ...
  const connection=directConnectionView(values[OPTIONS_DIRECT_STATUS_KEY]);
  const blockers=["Live execution adapter is not verified", "Fill recovery and automatic position exits are not operational"];
  if(!connection.lastCheckOk)blockers.unshift("Direct account collection is not currently verified");
  blockers.push(connection.recovery);
  ...
  return Response.json({at:new Date().toISOString(),connection,execution:{armed:values.options_live_armed==="true",canPlaceOrders:false,blockers},maxLoss,riskPct:account&&maxLoss?maxLoss/account.totalValue*100:null,rules:OPTIONS_DESK_RULES,watchlist,
