...
// call, no fetch; every number here was written by a cron and is dated. btcShock and
// derivatives are reserved for the steps that build them.
...
export async function GET() {
  try {
    ...
    const regime = parse<{ at: string; btcMtf: string | null; ethMtf: string | null; dataIssues: number; eventMode: string | null }>(c.kraken_margin_intel_latest);
    const riskState = parse<RiskState>(c.kraken_margin_risk_state);
    return Response.json({ eventPolicy, btcShock: null, derivatives: null, regime, riskState, at: new Date().toISOString() });
  } catch (error) {
