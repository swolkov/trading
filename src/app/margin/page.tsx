...
export default function MarginCockpitPage() {
  ...
  const { data: score } = useSWR<{ scoreboard: Scoreboard; recentTrips: Trip[]; shadow: ShadowScore | null; strategies: StrategyStat[]; log: PaperTradeRow[] }>(
    ...
    <div className="space-y-5">
      ...
      {score?.log && score.log.length > 0 && (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
                ...
                {score.log.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b border-border/20 last:border-0">
                      ...
                      <td className="text-left px-2 py-1.5 text-muted-foreground/70">{t.source === "scanner" ? "scanner" : "manual"}</td>
          ...
          <p className="text-[10px] text-muted-foreground/40 px-4 py-2 border-t border-border/50">
            Every paper trade the system tracked, labeled by strategy and conviction. Nothing here moved real money. Your <span className="text-foreground/60">real</span> completed trades are in the track record below.
          </p>
        </div>
      )}
