      ...
      {score?.strategies && score.strategies.some((s) => s.resolved > 0 || s.open > 0) && (
        <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <table className="w-full text-[11px]">
              <thead>
                <tr className="text-[9px] uppercase tracking-wider text-muted-foreground/50 border-b border-border/50">
                  ...
                  <th className="text-right font-medium px-2 py-1.5 text-foreground/70" title="The SAME trades priced the way the LIVE executor would size them: the agreed 3% risk, but FLAT — live has no conviction concept, while paper bets 2x on high conviction and 0.5x on low. This is what you would actually have made.">At LIVE sizing</th>
          ...
          <p className="text-[10px] text-muted-foreground/40 px-4 py-2 border-t border-border/50">
            <span className="text-foreground/60">Gross</span> is the raw edge (before fees); <span className="text-red-400/70">Fees</span> is the drag; <span className="text-foreground/60">Net</span> is what you keep. This is the exact battle that sank your real trading — your gross was ~break-even, but fees were the whole loss. A strategy only earns if <span className="text-foreground/60">Gross beats Fees</span>. Maker entries + fewer/bigger trades shrink the Fees column. <span className="text-foreground/60">At LIVE sizing</span> is the number that matters for a real decision. Both paper and live risk <span className="text-foreground/60">3%</span> per trade — the difference is that paper scales that by <span className="text-foreground/60">conviction</span> (2× on high, 0.5× on low) while the live executor has no conviction concept and bets flat. Across the surviving strategies that difference <em>is</em> the apparent edge: <span className="text-emerald-400/80">+$1,779</span> paper versus <span className="text-red-400/80">−$137</span> live on the same 48 trades. Until conviction is wired into the live path, the live column is the honest one. <span className="text-foreground/60">Green banked</span> is the give-back meter: what % of trades went green at their peak → what % finished green. A big gap means the strategy finds winners but hands them back — your August pattern (96% peaked green, 19% kept).
          </p>
        </div>
      )}
