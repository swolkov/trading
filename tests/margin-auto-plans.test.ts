    ...
    for (const tf of tfs) {
      for (const conv of convs) {
        for (const p of autoShadowPlans(kind, tf, conv, 5)) {
          ...
          assert.ok(["selective", "selective-x5", "selective-tight", "selective-launch", "swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr", "swing-partial"].includes(p.source), p.source);
        }
      }
    }
  }
...
test("1h/4h/1d high longs are paused — 3%/48h selective is a 5m/15m container", () => {
  ...
  // Sep 15 2026: swing-partial rides every 4h signal; swing-retest / swing-mtf / swing-atr need an
  // input (level / MTF read / ATR) and open nothing from a legacy six-argument call — deliberate.
  assert.deepEqual(autoShadowPlans("breakout", "4h", high, 5), [{ source: "swing-lev", lev: 5 }, { source: "swing-spot", lev: 1 }, { source: "swing-wide", lev: 5 }, { source: "swing-lock", lev: 5 }, { source: "swing-pyr", lev: 5 }, { source: "swing-partial", lev: 5 }], "4h high breakouts feed the slow family (reactivated Sep 8) plus the wide-trail twin (Sep 9), the lock twin (Sep 11), the pyramid twin (Sep 12) and the partial twin (Sep 15)");
...
test("the slow family (reactivated Sep 8) opens ONLY on high-conviction 4h/1d breakouts, longs, and never the fast twins", () => {
  assert.deepEqual(autoShadowPlans("breakout", "4h", high, 5).map((p) => p.source), ["swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr", "swing-partial"]);
  ...
  assert.deepEqual(autoShadowPlans("breakout", "4h", highStretched, 5).map((p) => p.source), ["swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr", "swing-partial"], "the Sep 3–4 rule had no stretched filter — kept, so the record stays continuous");
  assert.deepEqual(autoShadowPlans("breakout", "4h", med, 5), []);
  // The slow family's LONG sleeves stay longs-only; the 4h breakdown now feeds ONLY the paper-only
  // swing-short sleeve (Sep 15), and only in a confirmed BTC down-regime.
  assert.deepEqual(autoShadowPlans("breakdown", "4h", high, 5, { btcUp: false }), [{ source: "swing-short", lev: 5 }], "4h breakdowns → swing-short only");
  assert.deepEqual(autoShadowPlans("breakdown", "4h", high, 5, { btcUp: true }), [], "no short in an up-regime");
  assert.deepEqual(autoShadowPlans("breakdown", "4h", high, 5), [], "no regime read → no short");
...
test("the leveraged swing container is 4h-only; the spot container keeps both timeframes", () => {
  ...
  assert.deepEqual(at("4h"), ["swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr", "swing-partial"]);
