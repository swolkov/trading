    ...
    for (const tf of tfs) {
      for (const conv of convs) {
        for (const p of autoShadowPlans(kind, tf, conv, 5)) {
          ...
          assert.ok(["selective", "selective-x5", "selective-tight", "selective-launch", "swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr"].includes(p.source), p.source);
        }
      }
    }
  }
...
test("1h/4h/1d high longs are paused — 3%/48h selective is a 5m/15m container", () => {
  ...
  assert.deepEqual(autoShadowPlans("breakout", "4h", high, 5), [{ source: "swing-lev", lev: 5 }, { source: "swing-spot", lev: 1 }, { source: "swing-wide", lev: 5 }, { source: "swing-lock", lev: 5 }, { source: "swing-pyr", lev: 5 }], "4h high breakouts feed the slow family (reactivated Sep 8) plus the wide-trail twin (Sep 9) and the tight-trail twin (Sep 11)");
...
test("the slow family (reactivated Sep 8) opens ONLY on high-conviction 4h/1d breakouts, longs, and never the fast twins", () => {
  assert.deepEqual(autoShadowPlans("breakout", "4h", high, 5).map((p) => p.source), ["swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr"]);
  ...
  assert.deepEqual(autoShadowPlans("breakout", "4h", highStretched, 5).map((p) => p.source), ["swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr"], "the Sep 3–4 rule had no stretched filter — kept, so the record stays continuous");
...
test("the leveraged swing container is 4h-only; the spot container keeps both timeframes", () => {
  ...
  assert.deepEqual(at("4h"), ["swing-lev", "swing-spot", "swing-wide", "swing-lock", "swing-pyr"]);
