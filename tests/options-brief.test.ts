...
test("a conditional line pins the numbers: structure, the close level, the SPY/QQQ and earnings clauses, and the Normal cap as the max loss incl. fee", () => {
  ...
  const w = b.watch[0];
  assert.equal(w.line, "enter long call 12 2026-10-16 only if WTCH closes above 12 with SPY/QQQ constructive and no earnings before 2026-10-16; max loss incl. fee $100.50");
