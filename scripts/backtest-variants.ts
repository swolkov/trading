...
// THE ENGINE LIVES IN scripts/lib/replay-engine.ts (extracted verbatim, Sep 15 2026) so this and
// replay-walkforward.ts score a trade identically. Bar cache: BARS_CACHE env (see replay-engine).
import { exitParams, type ExitProfile } from "../src/lib/margin-shadow";
import type { KrakenBar } from "../src/lib/kraken-margin";
import { CACHE, MAKER, TAKER, entries, loadCache, money, nonOverlapping, report, simulate, simulatePyramid, tOf, type Entry } from "./lib/replay-engine";

async function main() {
  const bars = loadCache(CACHE);
