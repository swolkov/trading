...
function markdown(): string {
  ...
  out.push(``);
  out.push(`**How to read this.** Every row carries n, the mean R with its 95% CI, and t. A verdict word appears ONLY where |t| ≥ 2 on the full sample; the \`gate\` column is \`validateCandidate\`'s status (\`reject\` / \`research\`) with the reasons it gave. \`research\` is not a promotion: a survivor is "candidate for its own Pine + EDGES PR after Fable review" and nothing more. Slippage per side is the desk's own journal model (\`SLIP_PTS_PER_SIDE\`): ${MARKETS.map((m) => `${m.symbol} ${m.entrySlippagePoints} (${m.slippageSource})`).join(" · ")}; commission $${COMMISSION_ROUND_TURN} round turn. Entries at the next bar's open, one position at a time, ambiguous bars resolve to the stop, roll-crossing entries skipped, roll-interrupted trades fail the gate. The replay also refuses an entry whose slipped price falls outside the entry bar's range (\`unpriceable\`, a gate failure on its own): on 5-minute bars the desk's per-side slippage exceeds many quiet 2011–2015 bars, so 30–45% of those signals are dropped — the quiet ones — which is the existing gate's conservatism, left as it is; the diagnostics table counts them.`);
  out.push(``);
