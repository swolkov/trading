...
def upsert(conn, latest):
    with conn.cursor() as c:
        for sym, (bid, ask, ts) in latest.items():
            c.execute("""INSERT INTO live_quotes(symbol,bid,ask,mid,ts,source,updated_at)
                VALUES(%s,%s,%s,%s,%s,'databento',now())
                ON CONFLICT(symbol) DO UPDATE SET bid=%s,ask=%s,mid=%s,ts=%s,source='databento',updated_at=now()""",
                (sym, bid, ask, (bid+ask)/2, ts, bid, ask, (bid+ask)/2, ts))
...
def stream_once(conn):
    ...
    for rec in client:
        ...
        if bid > 0 and ask > 0:
            latest[sym] = (bid, ask, int(getattr(rec, "ts_event", time.time_ns()) // 1_000_000))
        now = time.time()
        if now - last_write >= WRITE_EVERY and latest:
            upsert(conn, latest); last_write = now
        if now - last_log >= 30:
            print(f"[sidecar] {', '.join(f'{s} {b:.2f}/{a:.2f}' for s,(b,a,_) in latest.items())}", flush=True)
            last_log = now
